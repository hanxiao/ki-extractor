...
<nav>
  ...
  <span class="slotpill" id="slotpill" style="display:none;font-size:11px;font-family:var(--mono);padding:1px 7px;border:1px solid var(--border-soft);border-radius:0;color:var(--text2)"></span>
...
<script>
// Shared L4 llama-server has 3 slots across dataroom/KG/searchbox/llamaui.
// Poll public slots endpoint (CORS-open) and show a compact nav pill when busy.
(function(){
  var U="https://hanxiao.io/llama-api/slots", el=document.getElementById("slotpill");
  if(!el) return;
  async function poll(){
    try{
      var s=await (await fetch(U,{cache:"no-store"})).json();
      var busy=s.filter(function(x){return x.is_processing;}).length, total=s.length;
      if(busy>=1){
        el.style.display="inline";
        var warn=busy>=total;
        el.textContent="\u25cf backend "+busy+"/"+total+" busy"+(warn?" \u00b7 queued":"");
        el.style.color=warn?"#b00":"var(--text2)";
        el.style.borderColor=warn?"#b00":"var(--border-soft)";
